...
func generateCommand(args []string) int {
	...
	if err := writeAIReadinessFiles(artifacts.OutputDir, inv); err != nil {
		fmt.Fprintf(os.Stderr, "Generate failed: %v\n", err)
		return 1
	}
...
func writeAIReadinessFiles(outDir string, inv *scanner.Inventory) error {
	if err := os.MkdirAll(filepath.Join(outDir, "examples"), 0o755); err != nil {
		return err
	}
	...
	agent := map[string]any{
		"name":        "reqit-repository-agent",
		"description": "Repository-native API intelligence output generated by reqit.",
		"generatedAt": time.Now().UTC().Format(time.RFC3339),
	}
	if b, err := json.MarshalIndent(agent, "", "  "); err == nil {
		if err := os.WriteFile(filepath.Join(outDir, "agent.json"), b, 0o644); err != nil {
			return err
		}
	}
	mcp := map[string]any{
		...
		"tools": []map[string]any{
			{"name": "scan", "description": "Scan repository and discover endpoints"},
			{"name": "drift", "description": "Detect OpenAPI contract drift"},
			{"name": "health", "description": "Compute repository API health score"},
		},
	}
	if b, err := json.MarshalIndent(mcp, "", "  "); err == nil {
		if err := os.WriteFile(filepath.Join(outDir, "mcp.json"), b, 0o644); err != nil {
			return err
		}
		if err := os.WriteFile(filepath.Join(outDir, "tool_manifest.json"), b, 0o644); err != nil {
			return err
		}
	}
	prompts := "# Reqit Prompts\n\n- Summarize endpoint coverage.\n- Flag breaking API changes.\n- Propose contract-safe updates.\n"
	if err := os.WriteFile(filepath.Join(outDir, "prompts.md"), []byte(prompts), 0o644); err != nil {
		return err
	}
	example := map[string]any{"endpointCount": len(inv.Endpoints), "authSchemes": inv.AuthSchemes}
	if b, err := json.MarshalIndent(example, "", "  "); err == nil {
		if err := os.WriteFile(filepath.Join(outDir, "examples", "inventory-summary.json"), b, 0o644); err != nil {
			return err
		}
	}
	schemaDoc := map[string]any{
		"$schema": "https://json-schema.org/draft/2020-12/schema",
		"title":   "ReqitInventorySummary",
		"type":    "object",
		"properties": map[string]any{
			"endpointCount": map[string]any{"type": "integer"},
			"authSchemes":   map[string]any{"type": "array", "items": map[string]any{"type": "string"}},
		},
	}
	if b, err := json.MarshalIndent(schemaDoc, "", "  "); err == nil {
		if err := os.WriteFile(filepath.Join(outDir, "schemas", "inventory-summary.schema.json"), b, 0o644); err != nil {
			return err
		}
	}
